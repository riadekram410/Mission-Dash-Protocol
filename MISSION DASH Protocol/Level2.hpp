...
void handlePageTransitions2() {
	current2 = clock();
	double duration = (double)(current2 - last2) / CLOCKS_PER_SEC;
	if (duration > 0.75) {
		last2 = current2;
		if (startPage == 1) {
			startPage = 2;
...
void keyPressLevel_2() {
	if (isKeyPressed('\r') && currentPage == 1) {
		currentPage = 2;
	}
	if (isSpecialKeyPressed(GLUT_KEY_RIGHT)) {
		handlePageTransitions2();
	}
